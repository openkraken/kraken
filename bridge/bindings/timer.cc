...
alibaba::jsa::Value setTimeout(alibaba::jsa::JSContext &rt,
  ...
  alibaba::jsa::Value* callbackValue = new alibaba::jsa::Value(args[0].getObject(rt));
...
alibaba::jsa::Value clearTimeout(alibaba::jsa::JSContext &rt,
  ...
  if (!timerId.isNumber()) {
    KRAKEN_LOG(WARN) << "[clearTimeout] clearTimeout accept number as parameter";
    return alibaba::jsa::Value::undefined();
  }
  ...
  if (callbackId == 0) {
    KRAKEN_LOG(WARN) << "[clearTimeout] can not stop timer of timerId: " << timer;
    return alibaba::jsa::Value::undefined();
  }
  ...
  if (callbackValue == nullptr || !callbackValue->getObject(rt).isFunction(rt)) {
    KRAKEN_LOG(WARN) << "[clearTimeout] can not stop timer of callbackId: " << callbackId;
    return alibaba::jsa::Value::undefined();
  }
...
void invokeSetTimeoutCallback(alibaba::jsa::JSContext *context, const int callbackId) {
  ...
  if (callback.isFunction(*context)) {
    callback.asFunction(*context).call(*context, alibaba::jsa::Value::undefined(), 0);
    delete callbackValue;
    timerCallbackMap.erase(callbackId);
  } else {
     KRAKEN_LOG(VERBOSE) << "callback is not a function";
