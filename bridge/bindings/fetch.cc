...
Value fetch(JSContext &context, const Value &thisVal, const Value *args, size_t count) {
  if (count != 3) {
  ...
  if (!func.getObject(context).isFunction(context)) {
    KRAKEN_LOG(WARN) << "__kraken_fetch__: first parameter should be an function";
    return Value::undefined();
  }
  ...
  // call request
  KrakenInvokeFetch(id, url.getString(context).utf8(context).c_str(), data.getString(context).utf8(context).c_str());
...
void invokeFetchCallback(alibaba::jsa::JSContext *context,
                         int callbackId,
  ...
  if (funcObj.isFunction(*context)) {
    ...
    funcObj.asFunction(*context).call(*context, jsError,
                                      response,
...
void bindFetch(alibaba::jsa::JSContext *context) {
  JSA_BINDING_FUNCTION(*context, context->global(), "__kraken__fetch__", 0, fetch);
}
