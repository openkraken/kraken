...
class NodeList : public GarbageCollected<NodeList> {
 public:
  ...
  uint32_t length() {
    return m_collection_index_cache.nodeCount(*this);
  };
  ...
  NodeInstance *traverseForwardToOffset(unsigned offset,
                                        NodeInstance &current_node,
                                        unsigned &current_offset) const;
  NodeInstance *traverseBackwardToOffset(unsigned offset,
                                         NodeInstance &current_node,
                                         unsigned &current_offset) const;
