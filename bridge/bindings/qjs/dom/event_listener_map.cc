...
static bool addListenerToVector(EventListenerVector* vector, JSValue callback) {
  if (std::find_if(vector->begin(), vector->end(), [&callback](JSValue fn) {
    return JS_VALUE_GET_PTR(fn) == JS_VALUE_GET_PTR(callback);
  }) != vector->end()) {
...
static bool removeListenerFromVector(
  ...
  // const on |listener|.
  auto it = std::find_if(
      listenerVector->begin(), listenerVector->end(),
      [&callback](const JSValue& listener)
          -> bool { return JS_VALUE_GET_PTR(listener) == JS_VALUE_GET_PTR(callback); });
