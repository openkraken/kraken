...
NodeInstance *NodeList::traverseForwardToOffset(unsigned int offset,
                                                NodeInstance &current_node,
                                                unsigned int &current_offset) const {
  assert(current_offset < offset);
//  assert(ownerNode().childNodes() == this);
//  DCHECK_EQ(&OwnerNode(), current_node.parentNode());
  for (NodeInstance* next = current_node.nextSibling(); next;
...
NodeInstance *NodeList::traverseBackwardToOffset(unsigned int offset,
                                                 NodeInstance &current_node,
                                                 unsigned int &current_offset) const {
  return nullptr;
}
