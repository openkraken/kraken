...
JSValue CSSStyleDeclaration::constructor(QjsContext *ctx, JSValue func_obj, JSValue this_val, int argc, JSValue *argv) {
  ...
  auto eventTargetInstance =
      static_cast<EventTargetInstance *>(JS_GetOpaque(eventTargetValue, kHostClassInstanceClassId));
...
JSValue CSSStyleDeclaration::setProperty(QjsContext *ctx, JSValue this_val, int argc, JSValue *argv) {
  if (argc < 2) return JS_ThrowTypeError(ctx, "Failed to execute 'setProperty' on 'CSSStyleDeclaration': 2 arguments required, but only 0 present.");
  auto *instance = static_cast<StyleDeclarationInstance *>(JS_GetOpaque(this_val, kHostClassInstanceClassId));
...
JSValue CSSStyleDeclaration::removeProperty(QjsContext *ctx, JSValue this_val, int argc, JSValue *argv) {
  if (argc < 1) return JS_ThrowTypeError(ctx, "Failed to execute 'removeProperty' on 'CSSStyleDeclaration': 1 arguments required, but only 0 present.");
  auto *instance = static_cast<StyleDeclarationInstance *>(JS_GetOpaque(this_val, kHostClassInstanceClassId));
...
JSValue CSSStyleDeclaration::getPropertyValue(QjsContext *ctx, JSValue this_val, int argc, JSValue *argv) {
  if (argc < 1) return JS_ThrowTypeError(ctx, "Failed to execute 'getPropertyValue' on 'CSSStyleDeclaration': 1 arguments required, but only 0 present.");
  auto *instance = static_cast<StyleDeclarationInstance *>(JS_GetOpaque(this_val, kHostClassInstanceClassId));
  JSValue &propertyNameValue = argv[0];
  const char* cPropertyName = JS_ToCString(ctx, propertyNameValue);
  std::string propertyName = std::string(cPropertyName);
