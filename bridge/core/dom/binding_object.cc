...
NativeValue BindingObject::InvokeBindingMethod(const AtomicString& method,
  ...
  if (binding_object_.invoke_bindings_methods_from_native == nullptr) {
    exception_state.ThrowException(context_->ctx(), RangeError, "Failed to call dart method: invokeBindingMethod not initialized.");
    return Native_NewNull();
  }

  NativeValue return_value = Native_NewNull();
  binding_object_.invoke_bindings_methods_from_native(&binding_object_, &return_value,  method.ToNativeString().release(), argc, argv);
  return return_value;
}
...
NativeValue BindingObject::GetBindingProperty(const AtomicString& prop, ExceptionState& exception_state) const {
  ...
  const NativeValue argv[] = {
    Native_NewString(prop.ToNativeString().release())
  };
  return InvokeBindingMethod(binding_call_methods::kgetPropertyMagic, 1, argv, exception_state);
}

NativeValue BindingObject::SetBindingProperty(const AtomicString& prop, NativeValue value, ExceptionState& exception_state) const {
  context_->dartMethodPtr()->flushUICommand();
  const NativeValue argv[] = {
      Native_NewString(prop.ToNativeString().release()),
      value
