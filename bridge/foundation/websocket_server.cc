...
public:
  ...
  void close(int code, const std::string &reason) override {
    if (m_server) {
      ...
      if (invalid_code) {
        KRAKEN_LOG(VERBOSE) << "[websocket] Server close code invalid. reset to 1000";
        code = websocketpp::close::status::normal;
      }

      m_server->close(m_hdl, static_cast<uint16_t>(code), reason, ec);
      if (ec) {
        KRAKEN_LOG(VERBOSE) << "[websocket] Server Error initiating close: " << ec.message();
      }
    }
  }

  void setLatch(foundation::CountDownLatch *latch) {
    this->m_latch = latch;
  }
...
public:
  WebSocketServerImpl() : is_listening(false) {
    ...
    m_endpoint.set_open_handler(
      websocketpp::lib::bind(&WebSocketServerImpl::_on_open, this, &m_endpoint, websocketpp::lib::placeholders::_1));

    m_endpoint.set_fail_handler(
      websocketpp::lib::bind(&WebSocketServerImpl::_on_fail, this, &m_endpoint, websocketpp::lib::placeholders::_1));
    m_endpoint.set_close_handler(
      websocketpp::lib::bind(&WebSocketServerImpl::_on_close, this, &m_endpoint, websocketpp::lib::placeholders::_1));
    m_endpoint.set_message_handler(websocketpp::lib::bind(
      &WebSocketServerImpl::_on_message, this, websocketpp::lib::placeholders::_1, websocketpp::lib::placeholders::_2));
  }
...
private:
  ...
  using session_list = std::map<websocketpp::connection_hdl, std::shared_ptr<WebSocketSessionImpl>,
...
void WebSocketServerImpl::_on_open(server *ser, websocketpp::connection_hdl hdl) {
  // new connection comes
  std::shared_ptr<WebSocketSessionImpl> session = std::make_shared<WebSocketSessionImpl>(ser, hdl);
...
void WebSocketServerImpl::_on_fail(server *ser, websocketpp::connection_hdl hdl) {
  auto con = ser->get_con_from_hdl(hdl);
  auto reason = con->get_ec().message();
  KRAKEN_LOG(VERBOSE) << "[websocket] connection establish failed because of " << reason;
}

void WebSocketServerImpl::_on_close(server *ser, websocketpp::connection_hdl hdl) {
  ...
  session_it->second->onClose(con->get_remote_close_code(), con->get_remote_close_reason());
...
void WebSocketServerImpl::_on_message(websocketpp::connection_hdl hdl, server::message_ptr msg) {
  session_list::iterator session_it = m_session_list.find(hdl);
  if (session_it == m_session_list.end()) {
    KRAKEN_LOG(VERBOSE) << "[websocket] No connection found with specific id";
    return;
...
bool WebSocketServerImpl::listen(int port, ConnectionCallback connectionCallback) {
  ...
  if (ec) {
    KRAKEN_LOG(ERROR) << "[websocket] Connect listen on port: " << port << " failed because of " << ec.message();
    return false;
  }
...
void WebSocketServerImpl::stopListening() {
...
void WebSocketServerImpl::stopListening(bool wait) {
  ...
  if (wait && m_session_list.size() > 0) {
    _latch = std::make_unique<foundation::CountDownLatch>(m_session_list.size());
  }
  ...
  for (auto it = m_session_list.begin(); it != m_session_list.end(); ++it) {
    ...
    websocketpp::lib::error_code ec;
    m_endpoint.close(it->second->get_hdl(), websocketpp::close::status::going_away, "going away", ec);
    if (ec) {
  ...
  if (ec) {
    KRAKEN_LOG(ERROR) << "[websocket] Server stop listening failed because of " << ec.message();
  }
