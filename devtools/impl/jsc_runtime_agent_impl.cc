...
#include "JavaScriptCore/heap/DeleteAllCodeEffort.h"
...
namespace kraken{
    namespace Debugger {
        ...
        /***************** RuntimeBackend *********************/
        void JSCRuntimeAgentImpl::awaitPromise(const std::string &in_promiseObjectId,
        ...
        bool JSCRuntimeAgentImpl::convertRemoteObject(const std::string& in_result,
        ...
        void JSCRuntimeAgentImpl::callFunctionOn(const std::string &in_functionDeclaration,
            ...
            Inspector::InjectedScript injectedScript = m_injectedScriptManager->injectedScriptForObjectId(in_objectId.fromJust().c_str());
            ...
            injectedScript.callFunctionOn(errorString,
                    in_objectId.fromJust().c_str(),
                    in_functionDeclaration.c_str(),
                    arguments.c_str(),
                    in_returnByValue.fromMaybe(false),
                    in_generatePreview.fromMaybe(false),
                    &out_result, &wasThrown);
        ...
        DispatchResponse JSCRuntimeAgentImpl::compileScript(const std::string &in_expression,
                                                            const std::string &in_sourceURL,
        ...
        DispatchResponse JSCRuntimeAgentImpl::enable() {
            ...
            m_frontend.executionContextCreated(
                    ExecutionContextDescription::create()
                    .setId(0)
                    .setName("default")
                    .setOrigin("default")
                    .setAuxData(nullptr)
        ...
        void JSCRuntimeAgentImpl::evaluate(const std::string &in_expression,
                                           Maybe<std::string> in_objectGroup,
...
//            *out_totalSize = 0;
        ...
        DispatchResponse JSCRuntimeAgentImpl::getProperties(const std::string &in_objectId,
            ...
            if(d.HasMember("type") && d["type"].IsString() && strcmp(d["type"].GetString(),"collection") == 0) {
                // map or set
                RefPtr<Inspector::Protocol::Array<Inspector::Protocol::Runtime::CollectionEntry>> collection_entries;
...
//                            .setPreview(std::move(preview))
            ...
            injectedScript.getProperties(errorString,
                                         objectId,
                                         in_ownProperties.fromMaybe(false),
                                         in_generatePreview.fromMaybe(false), &result);

            injectedScript.getInternalProperties(errorString,
                                                 objectId,
                                                 in_generatePreview.fromMaybe(false),
                                                 &internalProperties);
...
//                KRAKEN_LOG(VERBOSE) << "[Runtime.getProperties] internalProperties: " << internalPropStr;
                ...
                internal_prop_obj.Parse(internalPropStr);
        ...
        void JSCRuntimeAgentImpl::runScript(const std::string &in_scriptId,
                                            Maybe<int> in_executionContextId,
                                            Maybe<std::string> in_objectGroup,
                                            Maybe<bool> in_silent,
                                            Maybe<bool> in_includeCommandLineAPI,
        ...
        void JSCRuntimeAgentImpl::terminateExecution(
                std::unique_ptr<TerminateExecutionCallback> callback) {
            // TODO
        }

        DispatchResponse JSCRuntimeAgentImpl::addBinding(const std::string &in_name,
                                                         Maybe<int> in_executionContextId) {
            return DispatchResponse::Error("not implement yet");
        }
        ...
        void JSCRuntimeAgentImpl::setTypeProfilerEnabledState(bool isTypeProfilingEnabled) {
            ...
            vm.whenIdle([&vm, isTypeProfilingEnabled]() {
                bool shouldRecompileFromTypeProfiler = (isTypeProfilingEnabled
                                                        ? vm.enableTypeProfiler()
                                                        : vm.disableTypeProfiler());
                if (shouldRecompileFromTypeProfiler)
                    vm.deleteAllCode(JSC::PreventCollectionAndDeleteAllCode);
            });
        }
        ...
        void JSCRuntimeAgentImpl::setControlFlowProfilerEnabledState(bool isControlFlowProfilingEnabled) {
            ...
            vm.whenIdle([&vm, isControlFlowProfilingEnabled]() {
                bool shouldRecompileFromControlFlowProfiler = (isControlFlowProfilingEnabled
