//
// Created by rowandjj on 2019/4/22.
//
...
#include "devtools/protocol/entry_preview.h"
#include "devtools/protocol/property_preview.h"
...
namespace kraken{
    namespace Debugger {
        ...
        public:
            ...
            void setSubtype(const std::string &value) { m_subtype = value; }
            ...
            std::vector<std::unique_ptr<PropertyPreview>> *getProperties() { return m_properties.get(); }

            void setProperties(
                    std::unique_ptr<std::vector<std::unique_ptr<PropertyPreview>>> value) {
                m_properties = std::move(value);
            }

            bool hasEntries() { return m_entries.isJust(); }

            std::vector<std::unique_ptr<EntryPreview>> *getEntries(
                    std::vector<std::unique_ptr<EntryPreview>> *defaultValue) {
            ...
            public:
                ...
                ObjectPreviewBuilder<STATE | OverflowSet> &setOverflow(bool value) {
                    static_assert(!(STATE & OverflowSet),
                                  "property overflow should not be set yet");
                    m_result->setOverflow(value);
                    return castState<OverflowSet>();
                }

                ObjectPreviewBuilder<STATE | PropertiesSet> &setProperties(
                        std::unique_ptr<std::vector<std::unique_ptr<PropertyPreview>>> value) {
                    static_assert(!(STATE & PropertiesSet),
                                  "property properties should not be set yet");
                    m_result->setProperties(std::move(value));
                    return castState<PropertiesSet>();
                }

                ObjectPreviewBuilder<STATE> &
                setEntries(std::unique_ptr<std::vector<std::unique_ptr<EntryPreview>>> value) {
