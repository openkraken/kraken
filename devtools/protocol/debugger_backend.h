...
// debugger抽象接口
//
// 这里是所有的debugger commands，events的实现在frontend.h
//
...
#include "devtools/protocol/script_position.h"
...
#include "devtools/protocol/call_argument.h"
...
namespace kraken{
    namespace Debugger {
        ...
        public:
            ...
            virtual DispatchResponse continueToLocation(std::unique_ptr<Location> in_location, Maybe<std::string> in_targetCallFrames) = 0;

            virtual DispatchResponse disable() = 0;
            virtual DispatchResponse enable(Maybe<double> in_maxScriptsCacheSize, std::string* out_debuggerId) = 0;
            virtual DispatchResponse evaluateOnCallFrame(const std::string& in_callFrameId,
            ...
            virtual DispatchResponse getPossibleBreakpoints(std::unique_ptr<Location> in_start,
            ...
            virtual DispatchResponse searchInContent(const std::string& in_scriptId,
            ...
            virtual DispatchResponse setBlackboxedRanges(const std::string& in_scriptId,
                                                     std::unique_ptr<std::vector<std::unique_ptr<ScriptPosition>>> in_positions) = 0;
            virtual DispatchResponse setBreakpoint(std::unique_ptr<Location> in_location,
                                                   Maybe<std::string> in_condition,
                                                   std::string* out_breakpointId,
                                                   std::unique_ptr<Location>* out_actualLocation) = 0;
            virtual DispatchResponse setBreakpointByUrl(int in_lineNumber,
            ...
            virtual DispatchResponse setBreakpointOnFunctionCall(const std::string& in_objectId,
                                                                 Maybe<std::string> in_condition,
            ...
            virtual DispatchResponse setScriptSource(const std::string& in_scriptId,
            ...
            virtual DispatchResponse setVariableValue(int in_scopeNumber,
