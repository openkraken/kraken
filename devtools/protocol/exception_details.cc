    ...
    namespace Debugger {
        ...
        rapidjson::Value ExceptionDetails::toValue(rapidjson::Document::AllocatorType& allocator) const {
            ...
            if (m_scriptId.isJust())
                result.AddMember("scriptId", m_scriptId.fromJust(), allocator);
            if (m_url.isJust())
                result.AddMember("url", m_url.fromJust(), allocator);
            if (m_stackTrace.isJust())
                result.AddMember("stackTrace", m_stackTrace.fromJust()->toValue(allocator), allocator);
            if (m_exception.isJust())
                result.AddMember("exception", m_exception.fromJust()->toValue(allocator), allocator);
            if (m_executionContextId.isJust())
                result.AddMember("executionContextId", m_executionContextId.fromJust(), allocator);
            return result;
        }
    }
}