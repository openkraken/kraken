...
namespace kraken{
    namespace Debugger {
        ...
        void RuntimeDispatcherImpl::enable(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {
        ...
        void RuntimeDispatcherImpl::disable(uint64_t callId, const std::string& method,jsonRpc::JSONObject message, ErrorSupport*) {
            std::unique_ptr<DispatcherBase::WeakPtr> weak = weakPtr();
        ...
        void RuntimeDispatcherImpl::awaitPromise(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {
        ...
        public:
            CallFunctionOnCallbackImpl(std::unique_ptr<DispatcherBase::WeakPtr> backendImpl,
                    uint64_t callId,
                    const std::string& method,
                    kraken::Debugger::jsonRpc::JSONObject message,
                    rapidjson::Document::AllocatorType& allocator)
                    : DispatcherBase::Callback(std::move(backendImpl), callId, method, std::move(message)),
...
//                DCHECK(response.status() == DispatchResponse::kError);
        ...
        void RuntimeDispatcherImpl::callFunctionOn(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport* errors) {
            ...
            m_backend->callFunctionOn(in_functionDeclaration,
                    std::move(in_objectId),
                    std::move(in_arguments),
                    std::move(in_silent),
                    std::move(in_returnByValue),
                    std::move(in_generatePreview),
                    std::move(in_userGesture),
                    std::move(in_awaitPromise),
                    std::move(in_executionContextId),
                    std::move(in_objectGroup),
                    std::move(callback));
            return;
        }

        void RuntimeDispatcherImpl::compileScript(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport* errors) {
            // Prepare input parameters.
            errors->push();
            ...
            if (response.status() == DispatchResponse::kSuccess) {
                ...
                if (out_exceptionDetails.isJust()) {
                    result.AddMember("exceptionDetails", out_exceptionDetails.fromJust()->toValue(m_json_doc.GetAllocator()), m_json_doc.GetAllocator());
                }
            }
            if (weak->get())
        ...
        void RuntimeDispatcherImpl::discardConsoleEntries(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {

        }

        class EvaluateCallbackImpl : public RuntimeBackend::EvaluateCallback, public DispatcherBase::Callback {
        public:
            EvaluateCallbackImpl(std::unique_ptr<DispatcherBase::WeakPtr> backendImpl,
                    uint64_t callId,
                    const std::string& method,
                    kraken::Debugger::jsonRpc::JSONObject message,
                    rapidjson::Document::AllocatorType& allocator)
            ...
            void sendSuccess(std::unique_ptr<Debugger::RemoteObject> result, Maybe<Debugger::ExceptionDetails> exceptionDetails) override{
                rapidjson::Value resultObject(rapidjson::kObjectType);
                resultObject.AddMember("result", result->toValue(m_allocator),m_allocator);
                if (exceptionDetails.isJust())
                    resultObject.AddMember("exceptionDetails",
                                           exceptionDetails.fromJust()->toValue(m_allocator),m_allocator);
                sendIfActive(std::move(resultObject), DispatchResponse::OK());
            }
...
//                DCHECK(response.status() == DispatchResponse::kError);
        ...
        void RuntimeDispatcherImpl::evaluate(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport* errors) {
            // Prepare input parameters.
            errors->push();
            ...
            std::unique_ptr<DispatcherBase::WeakPtr> weak = weakPtr();
            std::unique_ptr<EvaluateCallbackImpl> callback(new EvaluateCallbackImpl(weakPtr(), callId, method, std::move(message), m_json_doc.GetAllocator()));
            m_backend->evaluate(in_expression,
                    std::move(in_objectGroup),
                    std::move(in_includeCommandLineAPI),
                    std::move(in_silent),
                    std::move(in_contextId),
                    std::move(in_returnByValue),
                    std::move(in_generatePreview),
                    std::move(in_userGesture),
                    std::move(in_awaitPromise),
                    std::move(in_throwOnSideEffect),
                    std::move(in_timeout),
                    std::move(callback));
            return;
        }

        void RuntimeDispatcherImpl::getIsolateId(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {
            // Declare output parameters.
            std::string out_id;
        ...
        void RuntimeDispatcherImpl::getHeapUsage(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {
            // Declare output parameters.
            double out_usedSize;
         ...
         *      - exceptionDetails
         * */
        void RuntimeDispatcherImpl::getProperties(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport* errors) {
            // Prepare input parameters.
            errors->push();
            ...
            std::unique_ptr<DispatcherBase::WeakPtr> weak = weakPtr();
            DispatchResponse response = m_backend->getProperties(in_objectId,
                                                                 std::move(in_ownProperties),
                                                                 std::move(in_accessorPropertiesOnly),
                                                                 std::move(in_generatePreview),
                                                                 &out_result,
                                                                 &out_internalProperties, &out_privateProperties, &out_exceptionDetails);
            if (response.status() == DispatchResponse::kFallThrough) {
                channel()->fallThrough(callId, method, std::move(message));
                return;
            }
        ...
        void RuntimeDispatcherImpl::globalLexicalScopeNames(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {

        }

        void RuntimeDispatcherImpl::queryObjects(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {

        }

        void RuntimeDispatcherImpl::releaseObject(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport* errors) {
            // Prepare input parameters.
            errors->push();
            std::string in_objectId = "";
            if(message.HasMember("objectId") && message["objectId"].IsString()) {
        ...
        void RuntimeDispatcherImpl::releaseObjectGroup(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport* errors) {
            // Prepare input parameters.
            errors->push();
        ...
        void RuntimeDispatcherImpl::runIfWaitingForDebugger(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {
            std::unique_ptr<DispatcherBase::WeakPtr> weak = weakPtr();
        ...
        void RuntimeDispatcherImpl::runScript(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {

        }

        void RuntimeDispatcherImpl::setCustomObjectFormatterEnabled(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {

        }

        void RuntimeDispatcherImpl::setMaxCallStackSizeToCapture(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {

        }

        void RuntimeDispatcherImpl::terminateExecution(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {

        }

        void RuntimeDispatcherImpl::addBinding(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {

        }

        void RuntimeDispatcherImpl::removeBinding(uint64_t callId, const std::string& method, jsonRpc::JSONObject message, ErrorSupport*) {
