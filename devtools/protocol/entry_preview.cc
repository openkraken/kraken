...
namespace kraken{
    namespace Debugger {
        std::unique_ptr<EntryPreview> EntryPreview::fromValue(rapidjson::Value *value,
        ...
        rapidjson::Value EntryPreview::toValue(
                rapidjson::Document::AllocatorType &allocator) const {
            rapidjson::Value result(rapidjson::kObjectType);
            if (m_key.isJust())
                result.AddMember("key", m_key.fromJust()->toValue(allocator), allocator);
