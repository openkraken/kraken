...
namespace kraken{
    namespace Debugger {
        ...
        public:
            ...
            std::string getScriptId() { return m_scriptId; }

            void setScriptId(const std::string &value) { m_scriptId = value; }
            ...
            void setHash(const std::string &value) { m_hash = value; }
            ...
            rapidjson::Value *getExecutionContextAuxData(rapidjson::Value *defaultValue) {
                return m_executionContextAuxData.isJust() ? m_executionContextAuxData.fromJust()
                                                          : defaultValue;
            }
            ...
            void setSourceMapURL(const std::string &value) { m_sourceMapURL = value; }
            ...
            public:
                enum {
                    ...
                    HashSet = 1 << 8,
                    AllFieldsSet = (ScriptIdSet | UrlSet | StartLineSet | StartColumnSet |
                                    EndLineSet | EndColumnSet | ExecutionContextIdSet | HashSet | 0)
                };


                ScriptFailedToParseNotificationBuilder<STATE | ScriptIdSet> &
                setScriptId(const std::string &value) {
                    static_assert(!(STATE & ScriptIdSet),
                                  "property scriptId should not be set yet");
                    m_result->setScriptId(value);
                    return castState<ScriptIdSet>();
                }

                ScriptFailedToParseNotificationBuilder<STATE | UrlSet> &
                setUrl(const std::string &value) {
                    static_assert(!(STATE & UrlSet), "property url should not be set yet");
                    m_result->setUrl(value);
                    return castState<UrlSet>();
                }

                ScriptFailedToParseNotificationBuilder<STATE | StartLineSet> &
                setStartLine(int value) {
                    static_assert(!(STATE & StartLineSet),
                                  "property startLine should not be set yet");
                    m_result->setStartLine(value);
                    return castState<StartLineSet>();
                }

                ScriptFailedToParseNotificationBuilder<STATE | StartColumnSet> &
                setStartColumn(int value) {
                    static_assert(!(STATE & StartColumnSet),
                                  "property startColumn should not be set yet");
                ...
                ScriptFailedToParseNotificationBuilder<STATE | EndColumnSet> &
                setEndColumn(int value) {
                    static_assert(!(STATE & EndColumnSet),
                                  "property endColumn should not be set yet");
                    m_result->setEndColumn(value);
                    return castState<EndColumnSet>();
                }

                ScriptFailedToParseNotificationBuilder<STATE | ExecutionContextIdSet> &
                setExecutionContextId(int value) {
                    static_assert(!(STATE & ExecutionContextIdSet),
                                  "property executionContextId should not be set yet");
                    m_result->setExecutionContextId(value);
                    return castState<ExecutionContextIdSet>();
                }

                ScriptFailedToParseNotificationBuilder<STATE | HashSet> &
                setHash(const std::string &value) {
                    static_assert(!(STATE & HashSet), "property hash should not be set yet");
                    m_result->setHash(value);
                    return castState<HashSet>();
                }

                ScriptFailedToParseNotificationBuilder<STATE> &
                setExecutionContextAuxData(std::unique_ptr<rapidjson::Value> value) {
                    m_result->setExecutionContextAuxData(std::move(value));
                    return *this;
                }

                ScriptFailedToParseNotificationBuilder<STATE> &
                setSourceMapURL(const std::string &value) {
                ...
                ScriptFailedToParseNotificationBuilder<STATE> &
                setStackTrace(std::unique_ptr<StackTrace> value) {
            ...
            private:
                ...
                ScriptFailedToParseNotificationBuilder() : m_result(
                        new ScriptFailedToParseNotification()) {}

                template<int STEP>
                ScriptFailedToParseNotificationBuilder<STATE | STEP> &castState() {
                    return *reinterpret_cast<ScriptFailedToParseNotificationBuilder<
                            STATE | STEP> *>(this);
                }
