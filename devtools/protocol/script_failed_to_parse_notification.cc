...
#include "script_failed_to_parse_notification.h"

namespace kraken{
    namespace Debugger {
        std::unique_ptr<ScriptFailedToParseNotification> ScriptFailedToParseNotification::fromValue(
            ...
            if(value->HasMember("hasSourceURL")) {
                ...
                if((*value)["hasSourceURL"].IsBool()) {
                    ...
                    errors->addError("hasSourceURL should be bool");
                }
            }
            ...
            if(value->HasMember("isModule")) {
                ...
                if((*value)["isModule"].IsBool()) {
                    ...
                    errors->addError("isModule should be bool");
                }
            }
            ...
            if(value->HasMember("stackTrace")) {
                ...
                if((*value)["stackTrace"].IsObject()) {
                    rapidjson::Value _stacktrace = (*value)["stackTrace"].GetObject();
                    result->m_stackTrace = StackTrace::fromValue(&_stacktrace, errors);
                } else {
        ...
        rapidjson::Value ScriptFailedToParseNotification::toValue(rapidjson::Document::AllocatorType& allocator) const {
            ...
            if (m_sourceMapURL.isJust())
                result.AddMember("sourceMapURL", m_sourceMapURL.fromJust(), allocator);
            if (m_hasSourceURL.isJust())
                result.AddMember("hasSourceURL", m_hasSourceURL.fromJust(), allocator);
            if (m_isModule.isJust())
                result.AddMember("isModule", m_isModule.fromJust(), allocator);
            if (m_length.isJust())
                result.AddMember("length", m_length.fromJust(), allocator);
