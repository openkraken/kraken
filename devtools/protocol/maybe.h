...
namespace kraken{
    namespace Debugger {
        ...
        public:
            ...
            T* fromJust() const {  return m_value.get(); }
            T* fromMaybe(T* defaultValue) const { return m_value ? m_value.get() : defaultValue; }
            bool isJust() const { return !!m_value; }
            std::unique_ptr<T> takeJust() {  return std::move(m_value); }
        private:
        ...
        template<>
        class Maybe<std::string> : public MaybeBase<std::string> {
        public:
            Maybe() { }
