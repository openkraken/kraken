...
namespace kraken{
    namespace Debugger {
        ...
        void DebugDispatcherImpl::dispatch(uint64_t callId,
         ...
         *
         * */
        void DebugDispatcherImpl::continueToLocation(uint64_t callId,
                                                     const std::string &method,
                                                     Debugger::jsonRpc::JSONObject message,
         ...
         *
         * */
        void DebugDispatcherImpl::disable(uint64_t callId,
                                          const std::string& method,
         ...
         *
         * **/
        void DebugDispatcherImpl::enable(uint64_t callId,
                                         const std::string& method,
                                         jsonRpc::JSONObject message,
                                         ErrorSupport*) {
            ...
            if(response.status() == DispatchResponse::kSuccess) {
                ...
                debugId.SetString(out_debuggerId, m_json_doc.GetAllocator());
         ...
         *
         * */
        void DebugDispatcherImpl::evaluateOnCallFrame(uint64_t callId,
                                                      const std::string& method,
                                                      jsonRpc::JSONObject message,
                                                      ErrorSupport* errors) {
            // Prepare input parameters.
         ...
         *
         * */
        void DebugDispatcherImpl::getPossibleBreakpoints(uint64_t callId,
                                                         const std::string& method,
                                                         jsonRpc::JSONObject message,
                                                         ErrorSupport* errors) {
            // Prepare input parameters.
         ...
         *
         * */
        void DebugDispatcherImpl::getScriptSource(uint64_t callId,
                                                  const std::string& method,
                                                  jsonRpc::JSONObject message,
                                                  ErrorSupport* errors) {
            // Prepare input parameters.
         ...
         *
         * */
        void DebugDispatcherImpl::getStackTrace(uint64_t callId,
                                                const std::string& method,
                                                jsonRpc::JSONObject message,
                                                ErrorSupport* errors) {
            // Prepare input parameters.
         ...
         *
         * */
        void DebugDispatcherImpl::pause(uint64_t callId,
                                        const std::string& method,
                                        jsonRpc::JSONObject message,
                                        ErrorSupport*) {
            std::unique_ptr<DispatcherBase::WeakPtr> weak = weakPtr();
        ...
        void DebugDispatcherImpl::pauseOnAsyncCall(uint64_t callId,
                                                   const std::string& method,
                                                   jsonRpc::JSONObject message,
                                                   ErrorSupport* errors) {
            // Prepare input parameters.
         ...
         * @param breakpointId
         * */
        void DebugDispatcherImpl::removeBreakpoint(uint64_t callId,
                                                   const std::string& method,
                                                   jsonRpc::JSONObject message,
                                                   ErrorSupport* errors) {
            // Prepare input parameters.
         ...
         * */
        void DebugDispatcherImpl::restartFrame(uint64_t callId,
                                               const std::string& method,
                                               jsonRpc::JSONObject message,
                                               ErrorSupport* errors) {
            // Prepare input parameters.
            ...
            if (response.status() == DispatchResponse::kSuccess) {
                ...
                if (out_asyncStackTrace.isJust()) {
                    result.AddMember("asyncStackTrace", out_asyncStackTrace.fromJust()->toValue(m_json_doc.GetAllocator()), m_json_doc.GetAllocator());
                }
                if (out_asyncStackTraceId.isJust()) {
                    result.AddMember("asyncStackTraceId", out_asyncStackTraceId.fromJust()->toValue(m_json_doc.GetAllocator()), m_json_doc.GetAllocator());
                }
            }
            if (weak->get())
                weak->get()->sendResponse(callId, response, std::move(result));
        ...
        void DebugDispatcherImpl::resume(uint64_t callId,
                                         const std::string& method,
                                         jsonRpc::JSONObject message,
                                         ErrorSupport*) {
            std::unique_ptr<DispatcherBase::WeakPtr> weak = weakPtr();
         ...
         *
         * */
        void DebugDispatcherImpl::searchInContent(uint64_t callId,
                                                  const std::string& method,
                                                  jsonRpc::JSONObject message,
                                                  ErrorSupport* errors) {
            // Prepare input parameters.
        ...
        void DebugDispatcherImpl::setAsyncCallStackDepth(uint64_t callId,
                                                         const std::string& method,
                                                         jsonRpc::JSONObject message,
                                                         ErrorSupport* errors) {
            // Prepare input parameters.
        ...
        void DebugDispatcherImpl::setBlackboxPatterns(uint64_t callId,
                                                      const std::string& method,
                                                      jsonRpc::JSONObject message,
                                                      ErrorSupport* errors) {
            // Prepare input parameters.
        ...
        void DebugDispatcherImpl::setBlackboxedRanges(uint64_t callId,
                                                      const std::string& method,
                                                      jsonRpc::JSONObject message,
                                                      ErrorSupport* errors) {
            // Prepare input parameters.
        ...
        void DebugDispatcherImpl::setBreakpoint(uint64_t callId,
                                                const std::string& method,
                                                jsonRpc::JSONObject message,
                                                ErrorSupport* errors) {
            // Prepare input parameters.
            ...
            if (response.status() == DispatchResponse::kSuccess) {
                ...
                result.AddMember("actualLocation", out_actualLocation.get()->toValue(m_json_doc.GetAllocator()), m_json_doc.GetAllocator());
            }
            if (weak->get())
                weak->get()->sendResponse(callId, response, std::move(result));
            return;
         ...
         *
         * */
        void DebugDispatcherImpl::setBreakpointByUrl(uint64_t callId,
                                                     const std::string& method,
                                                     jsonRpc::JSONObject message,
                                                     ErrorSupport* errors) {
            // Prepare input parameters.
        ...
        void DebugDispatcherImpl::setBreakpointOnFunctionCall(uint64_t callId,
                                                              const std::string& method,
                                                              jsonRpc::JSONObject message,
                                                              ErrorSupport* errors) {
            // Prepare input parameters.
        ...
        void DebugDispatcherImpl::setBreakpointsActive(uint64_t callId,
                                                       const std::string& method,
                                                       jsonRpc::JSONObject message,
                                                       ErrorSupport* errors) {
        ...
        void DebugDispatcherImpl::setPauseOnExceptions(uint64_t callId,
                                                       const std::string& method,
                                                       jsonRpc::JSONObject message,
                                                       ErrorSupport* errors) {
        ...
        void DebugDispatcherImpl::setReturnValue(uint64_t callId,
                                                 const std::string& method,
                                                 jsonRpc::JSONObject message,
                                                 ErrorSupport* errors) {
            // Prepare input parameters.
        ...
        void DebugDispatcherImpl::setScriptSource(uint64_t callId,
                                                  const std::string& method,
                                                  jsonRpc::JSONObject message,
                                                  ErrorSupport* errors) {
            // Prepare input parameters.
        ...
        void DebugDispatcherImpl::setSkipAllPauses(uint64_t callId,
                                                   const std::string& method,
                                                   jsonRpc::JSONObject message,
                                                   ErrorSupport* errors) {
            // Prepare input parameters.
        ...
        void DebugDispatcherImpl::setVariableValue(uint64_t callId,
                                                   const std::string& method,
                                                   jsonRpc::JSONObject message,
                                                   ErrorSupport* errors) {
            // Prepare input parameters.
        ...
        void DebugDispatcherImpl::stepInto(uint64_t callId,
                                           const std::string& method,
                                           jsonRpc::JSONObject message,
                                           ErrorSupport* errors) {
        ...
        void DebugDispatcherImpl::stepOut(uint64_t callId,
                                          const std::string& method,
                                          jsonRpc::JSONObject message,
                                          ErrorSupport*) {
        ...
        void DebugDispatcherImpl::stepOver(uint64_t callId,
                                           const std::string& method,
                                           jsonRpc::JSONObject message,
                                           ErrorSupport*) {
