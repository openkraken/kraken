    ...
    namespace Debugger {
        ...
        rapidjson::Value LogEntry::toValue(rapidjson::Document::AllocatorType &allocator) const {
            ...
            if (m_lineNumber.isJust())
                result.AddMember("lineNumber",m_lineNumber.fromJust(), allocator);
            ...
            if (m_networkRequestId.isJust())
                result.AddMember("networkRequestId", m_networkRequestId.fromJust(), allocator);
            if (m_workerId.isJust())
                result.AddMember("workerId",m_workerId.fromJust(), allocator);
            return result;
        }
    }
}