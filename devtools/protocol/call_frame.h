...
#include "devtools/protocol/scope.h"
...
namespace kraken{
    namespace Debugger {
        ...
        public:
            ...
            std::string getCallFrameId() { return m_callFrameId; }

            void setCallFrameId(const std::string &value) { m_callFrameId = value; }

            std::string getFunctionName() { return m_functionName; }

            void setFunctionName(const std::string &value) { m_functionName = value; }
            ...
            Location *getLocation() { return m_location.get(); }

            void setLocation(std::unique_ptr<Location> value) { m_location = std::move(value); }

            std::string getUrl() { return m_url; }

            void setUrl(const std::string &value) { m_url = value; }

            std::vector<std::unique_ptr<Scope>> *getScopeChain() { return m_scopeChain.get(); }

            void setScopeChain(std::unique_ptr<std::vector<std::unique_ptr<Scope>>> value) {
                m_scopeChain = std::move(value);
            }

            RemoteObject *getThis() { return m_this.get(); }

            void setThis(std::unique_ptr<RemoteObject> value) { m_this = std::move(value); }
            ...
            public:
                enum {
                    ...
                    ScopeChainSet = 1 << 5,
                    ThisSet = 1 << 6,
                    AllFieldsSet = (CallFrameIdSet | FunctionNameSet | LocationSet | UrlSet |
                                    ScopeChainSet | ThisSet | 0)
                };


                CallFrameBuilder<STATE | CallFrameIdSet> &setCallFrameId(const std::string &value) {
                    static_assert(!(STATE & CallFrameIdSet),
                                  "property callFrameId should not be set yet");
                    m_result->setCallFrameId(value);
                    return castState<CallFrameIdSet>();
                }

                CallFrameBuilder<STATE | FunctionNameSet> &
                setFunctionName(const std::string &value) {
                    static_assert(!(STATE & FunctionNameSet),
                                  "property functionName should not be set yet");
                ...
                CallFrameBuilder<STATE | LocationSet> &
                setLocation(std::unique_ptr<Location> value) {
                    static_assert(!(STATE & LocationSet),
                                  "property location should not be set yet");
                ...
                CallFrameBuilder<STATE | ScopeChainSet> &
                setScopeChain(std::unique_ptr<std::vector<std::unique_ptr<Scope>>> value) {
                    static_assert(!(STATE & ScopeChainSet),
                                  "property scopeChain should not be set yet");
