...
namespace kraken{
    namespace Debugger {
        ...
        rapidjson::Value PropertyDescriptor::toValue(rapidjson::Document::AllocatorType &allocator) const {
            ...
            if (m_value.isJust())
                result.AddMember("value", m_value.fromJust()->toValue(allocator), allocator);
            if (m_writable.isJust())
                result.AddMember("writable", m_writable.fromJust(), allocator);
            if (m_get.isJust())
                result.AddMember("get", m_get.fromJust()->toValue(allocator), allocator);
            if (m_set.isJust())
                result.AddMember("set", m_set.fromJust()->toValue(allocator), allocator);
            ...
            if (m_wasThrown.isJust())
                result.AddMember("wasThrown", m_wasThrown.fromJust(), allocator);
            if (m_isOwn.isJust())
                result.AddMember("isOwn", m_isOwn.fromJust(), allocator);
            if (m_symbol.isJust())
                result.AddMember("symbol", m_symbol.fromJust()->toValue(allocator), allocator);
            return result;
        }
    }
}