...
- (instancetype)initWithCameraName:(NSString *)cameraName
  ...
  NSLog(@"_captureDevice: %@", _captureDevice);
...
- (void)startVideoRecordingAtPath:(NSString *)path result:(FlutterResult)result {
  ...
  NSLog(@"_isRecording %@", _isRecording);
  if (!_isRecording) {
...
- (void)handleMethodCallAsync:(FlutterMethodCall *)call result:(FlutterResult)result {
  if ([@"availableCameras" isEqualToString:call.method]) {
    ...
    NSLog(@"MacOS: initialize cameraName: %@", cameraName);
    NSLog(@"MacOS: initialize resolutionPreset: %@", resolutionPreset);
    NSLog(@"MacOS: initialize enableAudio: %@", enableAudio);
    NSError *error;
