...
bool FlutterWindowController::CreateWindow(
  ...
  controller_ = FlutterDesktopCreateWindow(
      ...
      arg_count > 0 ? &engine_arguments[0] : nullptr, arg_count, hooks);
  if (!controller_) {
...
//kraken: 暴露ui线程post方法
void FlutterWindowController::PostUIThreadTask(KrakenVoidCallback callback, void* callback_data) {
    FlutterDesktopPostUIThreadTask(controller_, callback, callback_data);
}
