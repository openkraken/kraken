...
//Kraken: 增加std::string& callback,目前仅用于JS Callback回调
typedef void (*StringCallback)(const char* /* user data */);
...
typedef struct {
  ...
  //Kraken: dart回调js
  StringCallback dart_to_js_callback;
...
//kraken: 暴露ui线程post方法
FLUTTER_EXPORT
FlutterEngineResult FlutterEnginePostUIThreadTask(FlutterEngine engine,
                                                  VoidCallback callback,
                                                  void* callback_data);
