...
{return InjectedScript.RemoteObject.createObjectPreviewForValue(value,true);},functionDetails:function(func,previewOnly)
{var details=InjectedScriptHost.functionDetails(func);if(!details)
return"Cannot resolve function details."; if("rawScopes"in details){if(previewOnly)
delete details.rawScopes;else{var objectGroupName=this._idToObjectGroupName[parsedFunctionId.id];var rawScopes=details.rawScopes;var scopes=[];delete details.rawScopes;for(var i=0;i<rawScopes.length;i++)
...
return details;},wrapObject:function(object,groupName,canAccessInspectedGlobalObject,generatePreview)
...
return result;},_getProperties:function(objectId,collectionMode,generatePreview,nativeGettersAsValues)
...
this.__defineGetter__("$"+i,bind(injectedScript._savedResult,injectedScript,i));for(let method of BasicCommandLineAPI.methods)
...
result.push(object[key]);return result;},];for(let method of BasicCommandLineAPI.methods)
method.toString=function(){return"function "+method.name+"() { [Command Line API] }";};return injectedScript;})
