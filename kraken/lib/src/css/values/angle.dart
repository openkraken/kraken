...
class CSSAngle {
  ...
  static double parseAngle(String rawAngleValue) {
    ...
    if (rawAngleValue.endsWith(DEG)) {
      angleValue = double.tryParse(rawAngleValue.split(DEG)[0]) ?? 0.0 * 2 * math.pi / 360;
    } else if (rawAngleValue.endsWith(GRAD)) {
      angleValue = double.tryParse(rawAngleValue.split(GRAD)[0]) ?? 0.0 * 2 * math.pi / 400;
    } else if (rawAngleValue.endsWith(RAD)) {
      angleValue = double.tryParse(rawAngleValue.split(RAD)[0]) ?? 0.0;
    } else if (rawAngleValue.endsWith(TURN)) {
      angleValue = double.tryParse(rawAngleValue.split(TURN)[0]) ?? 0.0 * 2 * math.pi;
    }
    return angleValue;
  }
}
