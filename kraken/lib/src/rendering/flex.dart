...
class _RunMetrics {
  ...
  late double _remainingFreeSpace = 0;
...
class RenderFlexLayout extends RenderLayoutBox {
  ...
  double _getIntrinsicSize({
    required FlexDirection sizingDirection,
    required double extent, // the extent in the direction that isn't the sizing direction
    required double Function(RenderBox child, double extent) childSize, // a method to find the size in the sizing direction
  }) {
  ...
  double _getMinMainAxisSize(RenderBoxModel child) {
    ...
    double? childMinWidth = childRenderStyle.minWidth;
    double? childMinHeight = childRenderStyle.minHeight;
    double? childWidth = childRenderStyle.width;
    double? childHeight = childRenderStyle.height;
    double? childIntrinsicRatio = child.intrinsicRatio;

    if (child is RenderBoxModel) {
      minWidth = childMinWidth != null ? childMinWidth : child.autoMinWidth;
      minHeight = childMinHeight != null ? childMinHeight : child.autoMinHeight;
    ...
    if (child is RenderIntrinsic && childIntrinsicRatio != null &&
      CSSFlex.isHorizontalFlexDirection(renderStyle.flexDirection) && childWidth == null
    ) {
      double transferredSize = childHeight != null ? childHeight * childIntrinsicRatio : child.intrinsicWidth!;
      minMainSize = math.min(contentSize, transferredSize);
    } else if (child is RenderIntrinsic && childIntrinsicRatio != null &&
      CSSFlex.isVerticalFlexDirection(renderStyle.flexDirection) && childHeight == null
    ) {
      double transferredSize = childWidth != null ? childWidth /childIntrinsicRatio : child.intrinsicHeight!;
      minMainSize = math.min(contentSize, transferredSize);
  ...
  void performLayout() {
    ...
    while (child != null) {
      ...
      if (child is RenderBoxModel && childParentData.isPositioned) {
        ...
        if (scrollContainer != null && scrollContainer.hasSize) {
  ...
  void _setMaxScrollableSizeForFlex(List<_RunMetrics> runMetrics) {
    ...
    for (_RunMetrics runMetric in runMetrics) {
      ...
      void iterateRunChildren(int targetId, _RunChild runChild) {
        ...
        for (RenderBox sibling in runChildrenList) {
          preSiblingsMainSize += isHorizontalDirection ? sibling.size.width : sibling.size.height;
        }
        ...
        scrollableMainSizeOfChildren.add(preSiblingsMainSize +
          (isHorizontalDirection ? childScrollableSize.width + childMarginLeft :
          childScrollableSize.height + childMarginTop)
        );
        scrollableCrossSizeOfChildren.add(
          isHorizontalDirection ? childScrollableSize.height + childMarginTop :
    ...
    double maxScrollableMainSizeOfChildren = isScrollContainer ? maxScrollableMainSizeOfLines :
      (isHorizontalDirection ? container.renderStyle.paddingLeft : container.renderStyle.paddingTop) +
    ...
    double maxScrollableCrossSizeOfChildren = isScrollContainer ? maxScrollableCrossSizeOfLines :
      (isHorizontalDirection ? container.renderStyle.paddingTop : container.renderStyle.paddingLeft) +
        maxScrollableCrossSizeOfLines;

    double containerContentWidth = size.width - container.renderStyle.borderLeft - container.renderStyle.borderRight;
    double containerContentHeight = size.height - container.renderStyle.borderTop - container.renderStyle.borderBottom;
    ...
    scrollableSize = isHorizontalDirection ? Size(maxScrollableMainSize, maxScrollableCrossSize) :
  ...
  void _alignChildren(
    ...
    for (int i = 0; i < runMetrics.length; ++i) {
      ...
      final bool flipMainAxis = !(_startIsTopLeft(renderStyle.flexDirection));
      ...
      while (runChild != null) {
        final RenderLayoutParentData childParentData = runChild.parentData as RenderLayoutParentData;
        if (childParentData.runIndex != i) break;
